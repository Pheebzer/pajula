...
export async function getAccessToken(credentials: string): Promise<string> {
  const data = await spotifyApiRequest<AccessTokenResponse>( AUTH_URL, 'POST',
    { 
...
export async function getPlaylistMetadata(accessToken: string, playlistId: string): Promise<MetadataResponse> {
  ...
  const data = await spotifyApiRequest<MetadataResponse>( url.toString(), 'GET', 
...
export async function getTrackPage(accessToken: string, playlistId: string, offset: number): Promise<TracksPageResponse> {
  ...
  url.searchParams.set(
    'fields',
    'items(added_by(id,display_name),added_at,track(name,id,duration_ms,album(name),artists(name)))'
  );
  url.searchParams.set('limit', '100');
  url.searchParams.set('offset', String(offset));
  const data = await spotifyApiRequest<TracksPageResponse>( url.toString(), 'GET', 
    {'Authorization': `Bearer ${accessToken}`}
  )
  return data;
}
...
export async function getUsernameById(accessToken: string, id: string) {
  ...
  const data = await spotifyApiRequest<UserDataResponse>( url.toString(), 'GET',
    {'Authorization': `Bearer ${accessToken}`}
  );
  return data;
}
